...
///
/// Returns 0 for zero-width codepoints, 2 for wide codepoints, and 1 for
/// everything else. This operates on a single codepoint only and therefore
/// cannot account for grapheme-cluster-level width rules. For cluster-accurate
/// widths, use [`grapheme_width`]. Summing per-codepoint widths is only correct
...
/// Returns the terminal display width of a Rust character in terminal grid cells.
///
/// This is equivalent to [`codepoint_width`] for the character's Unicode scalar
/// value.
#[must_use]
pub fn char_width(ch: char) -> u8 {
    codepoint_width(u32::from(ch))
}
...
///
/// Reads codepoints until the terminal would consider the grapheme cluster
/// complete and returns the number of codepoints consumed with the cluster's
...
/// chunks, keep buffering while this function consumes all available codepoints
/// and the stream may still continue; a later codepoint could still extend the
